...
function setQueryStirng() {
    ...
    for (var i = 0; i < vars.length; i++) {
        ...
        var val = decodeURIComponent(pair[1]).replace(new RegExp('[<>\(\)]', 'gim'), function (c) {
...
$.fn.val = function (val) {
    ...
    if (target && (T.data().select2 || T.is('select')) && val !== undefined && args[2] === undefined) {
        ...
        if (Array.isArray(val) && T.prop('multiple')) {
            ...
            if (searchIds.length > 0 && T.is(':visible')) {
...
function getTextKeyValue(o, T) {
    ...
    if (T && T.data('view-key'))
        options.push.apply(options, T.data('view-key').split(','))
...
function createSimblaObj(form) {
    ...
    function save() {
        ...
        function done(refresh) {
            ...
            if (obj.className !== QueryString.cls && !T.is('.dont-reset') && !T.data('dont-reset')) {
                form.reset();
                T.removeData('val')
        ...
        m.find('table.table thead th').each(function () {
            ...
            if (m.data('is-aggregate')) {
                ...
                if ((fieldType === "Number" && $(this).data('aggrFunc')) || $(this).data('aggrFunc') === "count") {
                    val = d[key];
                } else if (fieldType === "Date" && typeof d._id[key] === "object" && $(this).data('aggrFunc')) {
    ...
    function getAggregateQuery(query) {
        ...
        m.find('table thead th').each(function () {
            ...
            if (data.type === "Number" && data.aggrFunc) {
                ...
                groupby[data.field.replace(/\./g, '__')][data.aggrFunc || "mm/yy"] = data.aggrField;
            } else {
...
function getSelectPointers(targetClass, selectfields, value, T, name) {
    ...
    function setResult(d) {
        ...
        d.forEach(function (o) {
            ...
            T.append($('<option>').data('obj', o).attr('value', o.id).text(decodeEntities(name ? (name == "objectId" ? o.id : o.get(name)) : getTextKeyValue(o, T))));
        });
        if (val != undefined) {
            if (T.prop('multiple')) {
                val = val.split(',')
